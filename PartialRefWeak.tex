...
\begin{alltt}
\pvsid{PartialRefWeak}\({\pvsbracketl}\)\pvsid{Conf}: \pvskey{TYPE}, \pvsid{FM}: \pvskey{TYPE}, \pvsid{\{||\}}: \({\pvsbracketl}\)\pvsid{FM} \(\rightarrow\) \pvsid{set}\({\pvsbracketl}\)\pvsid{Conf}\({\pvsbracketr}\)\({\pvsbracketr}\), \pvsid{Asset}: \pvskey{TYPE}, \pvsid{AssetName}: \pvskey{TYPE},
 ...
 \pvskey{BEGIN}
  ...
  \pvsid{CM}: \pvskey{TYPE} = \pvsid{maps}\({\pvsbracketl}\)\pvsid{Conf}, \pvsid{Conf}\({\pvsbracketr}\).\pvsid{mapping}\vspace*{\pvsdeclspacing}

  \(c\): \pvskey{VAR} \pvsid{Conf}\vspace*{\pvsdeclspacing}

  \(m\), \(n\): \pvskey{VAR} \pvsid{CM}\vspace*{\pvsdeclspacing}

  \pvskey{IMPORTING} \pvsid{maps\_identity}\({\pvsbracketl}\)\pvsid{Conf}\({\pvsbracketr}\)

  \pvskey{IMPORTING} \pvsid{maps\_composite}\({\pvsbracketl}\)\pvsid{Conf}, \pvsid{Conf}, \pvsid{Conf}\({\pvsbracketr}\)
  ...
  \pvsid{pl}, \pvsid{pl1}, \pvsid{pl2}, \pvsid{pl3}: \pvskey{VAR} \pvsid{PL}\vspace*{\pvsdeclspacing}

  \pvsid{weakPartialRefinement}\pvsid{(}\pvsid{pl1}, \pvsid{pl2}, \(m\)\pvsid{)}: \pvsid{bool} \pvskey{=}
      \(\setsothersubsetothertwofn{\pvsid{domain}\pvsid{(}m\pvsid{)}}{\pvsid{\{||\}}\pvsid{(}F\pvsid{(}\pvsid{pl1}\pvsid{)}\pvsid{)}}\) \(\wedge\)
       \(\setsothersubsetothertwofn{\pvsid{image}\pvsid{(}m\pvsid{)}}{\pvsid{\{||\}}\pvsid{(}F\pvsid{(}\pvsid{pl2}\pvsid{)}\pvsid{)}}\) \(\wedge\)
        \pvsid{(}\pvskey{forall} \(c\):
            \pvsid{domain}\pvsid{(}\(m\)\pvsid{)}\pvsid{(}\(c\)\pvsid{)} \(\Rightarrow\)
             \pvsid{(}\pvsid{prod}\pvsid{(}\pvsid{pl1}, \(c\)\pvsid{)} \pvsid{|-} \pvsid{prod}\pvsid{(}\pvsid{pl2}, \pvsid{getRight}\pvsid{(}\(m\), \(c\)\pvsid{)}\pvsid{)}\pvsid{)}\pvsid{)}\vspace*{\pvsdeclspacing}

  \pvsid{weakPartRefReflexive}: \pvskey{THEOREM}
    \pvskey{forall} \pvsid{pl}, \pvsid{(}\(m\): \pvsid{CM} | \(\setsothersubsetothertwofn{\pvsid{domain}\pvsid{(}m\pvsid{)}}{\pvsid{\{||\}}\pvsid{(}F\pvsid{(}\pvsid{pl}\pvsid{)}\pvsid{)}}\)\pvsid{)}:
      \pvsid{identity?}\pvsid{(}\(m\)\pvsid{)} \(\Rightarrow\) \pvsid{weakPartialRefinement}\pvsid{(}\pvsid{pl}, \pvsid{pl}, \(m\)\pvsid{)}\vspace*{\pvsdeclspacing}

  \pvsid{weakPartRefTransitive}: \pvskey{THEOREM}
    \pvskey{forall} \pvsid{pl1}, \pvsid{pl2}, \pvsid{pl3}, \(m\), \(n\):
      \pvsid{(}\pvsid{(}\pvsid{weakPartialRefinement}\pvsid{(}\pvsid{pl1}, \pvsid{pl2}, \(m\)\pvsid{)} \(\wedge\)
           \pvsid{weakPartialRefinement}\pvsid{(}\pvsid{pl2}, \pvsid{pl3}, \(n\)\pvsid{)} \(\wedge\) \pvsid{image}\pvsid{(}\(m\)\pvsid{)} \(=\) \pvsid{domain}\pvsid{(}\(n\)\pvsid{)}\pvsid{)}
         \(\Rightarrow\) \pvsid{weakPartialRefinement}\pvsid{(}\pvsid{pl1}, \pvsid{pl3}, \(q\)\pvsid{)}\pvsid{)}
        \pvskey{WHERE} \(q\) \pvskey{=} \pvsid{composeMaps}\pvsid{(}\(m\), \(n\)\pvsid{)}\vspace*{\pvsdeclspacing}

 \pvskey{END} \pvsid{PartialRefWeak}\end{alltt}
