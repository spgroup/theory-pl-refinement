...
\begin{alltt}
\pvsid{Assets}: \pvskey{THEORY}
 \pvskey{BEGIN}

  \pvskey{IMPORTING} \pvsid{set\_aux\_lemmas}

  \pvsid{Asset}: \pvskey{TYPE+}\vspace*{\pvsdeclspacing}

  \pvsid{AssetName}: \pvskey{TYPE+}\vspace*{\pvsdeclspacing}

  \pvskey{CONVERSION+} \pvsid{singleton}


  \(a\), \(a\sb{1}\), \(a\sb{2}\), \(a\sb{3}\): \pvskey{VAR} \pvsid{Asset}\vspace*{\pvsdeclspacing}
  ...
  \pvsid{AssetTest}: \pvskey{THEOREM}
    \pvskey{forall} \pvsid{(}\(S\), \(T\), \(x\), \(y\): \pvsid{finite\_sets}\({\pvsbracketl}\)\pvsid{Asset}\({\pvsbracketr}\).\pvsid{finite\_set}, \(a\), \(b\): \pvsid{Asset}\pvsid{)}:
      \pvsid{wfProduct}\pvsid{(}\(x\)\pvsid{)} \(\wedge\)
       \pvsid{(}\(S\) \pvsid{|-} \(T\)\pvsid{)} \(\wedge\)
        \(x\)\pvsid{(}\(a\)\pvsid{)} \(\wedge\)
         \pvsid{(}\pvsid{singleton}\({\pvsbracketl}\)\pvsid{Asset}\({\pvsbracketr}\)\pvsid{(}\(a\)\pvsid{)} \pvsid{|-} \pvsid{singleton}\({\pvsbracketl}\)\pvsid{Asset}\({\pvsbracketr}\)\pvsid{(}\(b\)\pvsid{)}\pvsid{)} \(\wedge\)
          \(x\) \(=\) \(\setsotheruniontwofn{S}{\pvsid{singleton}{\pvsbracketl}\pvsid{Asset}{\pvsbracketr}\pvsid{(}a\pvsid{)}}\) \(\wedge\) \(y\) \(=\) \(\setsotheruniontwofn{T}{\pvsid{singleton}{\pvsbracketl}\pvsid{Asset}{\pvsbracketr}\pvsid{(}b\pvsid{)}}\)
       \(\Rightarrow\) \pvsid{(}\(x\) \pvsid{|-} \(y\)\pvsid{)}\vspace*{\pvsdeclspacing}

 \pvskey{END} \pvsid{Assets}\end{alltt}
